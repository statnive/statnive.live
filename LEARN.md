...
- [A. Build & release](#a-build--release)
  ...
  - [32 — Never `gh release create vX.Y.Z` manually after pushing a tag — `release.yml` owns the create step](#lesson-32)
...
- [L. Authorization & multi-tenant access](#l-authorization--multi-tenant-access)
  ...
  - [40 — A new table with a subject-identity column must be wired into an erase path in the SAME change; never write a migration comment claiming erasure that doesn't exist](#lesson-40)
  - [42 — The OpenAPI contract is generated from the chi router + a hand-authored overlay; never hand-edit the merged spec, and never POST-then-GET-stats when verifying live (1h rollup delay)](#lesson-42)
...
**Never `gh release create vX.Y.Z` manually after pushing a tag — `release.yml` owns the create step. A pre-existing empty release blocks the asset upload, leaves the release without artifacts, and `deploy-saas.yml` (which fires on `release: published`) errors with "no assets to download" then exits without ever shipping a binary.**

1. **What we did** — Shipping v0.0.1 (first non-RC release): merged the landing PR, ran `git tag -a v0.0.1 -m "..." && git push origin v0.0.1`, then immediately ran `gh release create v0.0.1 --title "..." --notes "..."` to "publish the release with nice notes." The tag-push triggered `release.yml` (build + sign + publish) in parallel; my `gh release create` finished in ~1 s and emitted a `release: published` event that fired `deploy-saas.yml` against an empty release. The deploy-saas run failed in 7 s at "Download release artifacts → no assets to download" (exit 1). Then `release.yml` finished its 4 min build, hit its own `gh release create v0.0.1 ... build/*.tar.gz build/SHA256SUMS build/SHA256SUMS.sig` step, and failed with `a release with the same tag name already exists: v0.0.1` (exit 1). Net effect: a tag, an empty release, two red workflow runs, and no shipped binary.
2. **Why it broke** — `.github/workflows/release.yml` is the canonical owner of the GitHub Release for any pushed `v*` tag. Its "Publish GitHub Release" step is `gh release create "$VERSION" --title "$VERSION" --notes "..." build/${BASE}.tar.gz build/SHA256SUMS build/SHA256SUMS.sig` — `create`, not `upload`. `gh release create` is non-idempotent: if the tag already has a release object, it errors. So an operator who runs `gh release create $TAG` racing the workflow always wins the race (workflow has to build first) and always breaks the workflow. There's no warning anywhere — the operator just sees their nicely-crafted release notes published, and only later notices the deploy never happened.
3. **The fix we applied** — In-flight recovery without re-tagging or re-running release.yml: (a) `gh run download <release-run-id> --name airgap-bundle-v0.0.1` pulled the signed bundle (14-day workflow-artifact retention saved us). (b) `gh release upload v0.0.1 statnive-live-v0.0.1-linux-amd64-airgap.tar.gz SHA256SUMS SHA256SUMS.sig` attached the assets to the empty release. (c) `gh workflow run deploy-saas.yml -f version=v0.0.1` re-fired the deploy via `workflow_dispatch` (the `release: published` event won't refire on asset upload). Deploy ran clean, /healthz green, /api/about reports `git_sha=<merge SHA>`, prod is on v0.0.1.
4. **Preventive measure** — Three layers. (a) **Operator rule**: after `git push origin v*`, **stop**. Watch `gh run watch $(gh run list --workflow=release.yml -L 1 --json databaseId -q '.[0].databaseId')` and let release.yml own the create. If you want better release notes than auto-generated, use `gh release edit v0.0.1 --notes "$(cat ...)"` AFTER release.yml has succeeded. (b) **Workflow rule**: `release.yml`'s Publish step should switch from `gh release create` to a two-step `gh release create --draft || true` then `gh release upload --clobber` so it's idempotent against an operator's pre-created draft. Track as a follow-up. (c) **Runbook**: `docs/runbook.md` § Release adds a "**DO NOT** `gh release create` manually" warning at the top of the release SOP, with a pointer back to this lesson.
...
**Authorization middleware that requires a per-resource query param (e.g. `?site_id=N`) must be permissive when the param is absent — global-scope admin routes (`/api/admin/sites` listing, `/api/admin/currencies`, `/api/admin/users/{uuid}/disable`) don't carry the param by design. Reject only when the param IS present and invalid (zero / non-numeric). Pin the passthrough contract with a unit test AND wire flag-ON behaviour into the boot smoke so the regression is caught at CI before a customer-facing 400 appears.**

1. **What we did** — v0.0.10 shipped `RequireSiteRole` middleware that read `?site_id=<n>` and returned 400 "bad site_id" when missing. Tests at the time pinned that contract (`TestRequireSiteRole_BadSiteID_400` asserted 400 for missing/empty/zero/non-numeric). Smoke harness ran with `STATNIVE_FEATURES_PER_SITE_ADMIN=false` (legacy `RequireRole`), so the per-site middleware was never exercised end-to-end. Production operator flipped the flag, opened the admin panel, every global-scope call returned 400. Admin panel showed the literal string "bad site_id" under the site-context bar.
2. **Why it broke** — Routes were mounted into one chi group under one middleware. The middleware enforced "every request needs `?site_id`" — but Sites.List / Options.Currencies / Options.Timezones / Users.{Disable,Enable,ResetPassword,Update,UpdateSites} all operate either at global scope or against a UUID path param. The SPA correctly omitted `?site_id` for those calls; the middleware rejected them. No CI gate caught it because the existing smoke harness booted with flag=OFF; no Vitest UI test exercised the actual fetch URLs; no integration test booted a flag=ON binary.
3. **The fix we applied** — (a) Rewrote `RequireSiteRole` to be permissive: when `?site_id` is absent, pass through to the handler so long as the actor has at least one qualifying admin grant somewhere (`hasAnyRole` floor check blocks revoked-everywhere users). When present, validate as before. (b) Updated `TestRequireSiteRole_BadSiteID_400` to only cover invalid values (zero/non-numeric), and added two new tests: `TestRequireSiteRole_NoSiteID_PassesThrough` (pins the regression-class fix) and `TestRequireSiteRole_NoGrantsAnywhere_403` (pins the floor check). (c) Flipped the smoke harness to boot with `STATNIVE_FEATURES_PER_SITE_ADMIN=true` so every existing admin probe now exercises the per-site path. (d) Extended `auth.Bootstrap` to also write a `user_sites` grant for the seeded admin on their bootstrap site_id — without this, the floor check would block the smoke admin from /api/admin/*. (e) Made `Users.Create` write a `user_sites` grant for newly-created users even when the body uses the legacy `role` shape (no `sites` array), so per-site `GET /api/admin/users` returns the row.
4. **Preventive measure** — Three layers of regression catch land together:
   - **Unit test** `TestRequireSiteRole_NoSiteID_PassesThrough` in `internal/auth/user_sites_test.go` — fails the moment future refactors restore the "always require site_id" behavior. Names the bug class so the failure message is self-documenting.
   - **Smoke harness flag-ON** in `test/smoke/harness.sh` — every existing admin probe (site creation, user CRUD, goal CRUD) now runs through `RequireSiteRole` end-to-end. A regression returns the same 400 in CI that prod surfaced, before merge.
   - **Bootstrap-time `user_sites` grant** in `internal/auth/bootstrap.go` — every fresh deploy gets a working operator admin under the per-site flag without manual SQL. Operators don't need to remember the post-deploy SSH step.

  PR review checklist: any change to `RequireSiteRole` body, `parseSiteID`, or the floor check MUST keep all three of these gates green. Drop the smoke flag-ON env only with an explicit `# flag-OFF coverage` comment justifying why.
...
**Per-site authorization must travel with the URL parameter, not just the session. v0.0.10 shipped `RequireSiteRole` on `/api/admin/*` but left the dashboard read routes (`/api/sites`, `/api/stats/*`, `/api/realtime/visitors`) under the legacy `RequireRole(admin, viewer, api)` middleware — which checks the actor's *role enum* but NOT their *per-site grant*. Result: a user granted admin on site_id=4 (televika.com) could swap `?site=4` to `?site=5` and read every other tenant's analytics. OWASP A01:2021 Broken Access Control / IDOR.**

1. **What we did** — Mounted `RequireSiteRole` on `/api/admin/*` for the per-site admin feature (v0.0.10, PR #105), but reused the old `RequireRole(admin/viewer/api)` middleware for the dashboard read group at `cmd/statnive-live/main.go:460-473`. Every dashboard handler called `filterFromRequest` (`internal/dashboard/filter.go:67`) which read `parseSiteID(q.Get("site"))` and trusted the result. `whereTimeAndTenant` (`internal/storage/queries.go:49`) — the Architecture Rule 8 choke point — happily scoped queries to whatever site_id it was handed. `/api/sites` (`internal/dashboard/sites_handler.go:24`) returned the full registry without filtering by `actor.SiteIDs()`. Three days of operator usage including a v0.0.10 per-site user-creation bug (Lesson 34 family) before the leak surfaced.

2. **Why it broke** — Session/bearer auth proves **who** the actor is, not **which sites** they can read. Architecture Rule 8 (`whereTimeAndTenant` always present) caught data-isolation bugs (rows from siteA appearing in a siteB query) but NOT authorization bugs (a siteA actor permitted to issue siteB queries). They are different leaks — Rule 8 is a SQL-layer invariant, OWASP A01 is an authorization-layer invariant. Both layers need explicit enforcement; the dashboard mount never got the latter.

3. **The fix we applied** — (a) New `auth.RequireDashboardSiteAccess` middleware in `internal/auth/dashboard_site_role.go`, sibling of `RequireSiteRole` but with three deliberate differences: reads `?site` (dashboard convention) not `?site_id`; **fails closed** when `?site` is missing; API-token actors (`UserID == uuid.Nil`) are scoped to `{token.SiteID: token.Role}` only — token capability never escalates to the underlying user's other grants. (b) Split `dashboard.Mount` into `MountSiteListing` (just `/api/sites`, filters inline by actor grants) + `MountSiteScoped` (all 10 stats/realtime routes, behind the new middleware). (c) `filterFromRequest` re-checks `actor.CanAccessSite` as belt-and-braces (`resolveAuthorizedSiteID` in `internal/dashboard/filter.go`) — a future handler that forgets the middleware still 403s. (d) `/api/sites` filters its response by `actor.SiteIDs()` so the SiteSwitcher dropdown only shows accessible tenants. (e) Audit events carry `site_id` via `dashboardEventAttrs`; new `EventDashboardForbidden` records denied requests for forensic correlation. (f) Legacy `PerSiteAdmin=OFF` deploys keep the `actor.SiteID == requestedSite` invariant. (g) Production CH backfill grants the canonical operator (`parhum.kh@gmail.com`) admin on every existing site_id so the legitimate all-sites admin path keeps working.

4. **Preventive measure** — Three layers of CI regression catch:
   - **Semgrep rule** `dashboard-site-query-needs-authz` in `.claude/skills/tenancy-choke-point-enforcer/semgrep/`. Flags any `r.URL.Query().Get("site")` (or `site_id`) in `internal/dashboard/**` that isn't routed through `filterFromRequest`, `resolveAuthorizedSiteID`, `ActiveSiteIDFromContext`, or an explicit `CanAccessSite` call. Wired into `make tenancy-grep` (which `make lint` and `make ci-local` depend on). Self-test target `make tenancy-grep-selftest` asserts the rule fires on the `should-trigger` fixtures.
   - **Smoke harness probe** `probe_per_site_authz` in `test/smoke/harness.sh`. Creates two sites + two viewer users (alpha/beta) and asserts every cross-tenant dashboard route returns 403. Any 200 fails the harness. Runs in `make smoke` (and therefore `make ci-local`).
   - **Unit-test matrix** — `internal/auth/dashboard_site_role_test.go` (11 middleware branches including api-token cross-site, legacy single-site, role-floor), `internal/dashboard/per_site_authz_test.go` (per-handler 403 + `Store.Calls==0` invariant proves rejection happens before any CH query), `internal/dashboard/sites_handler_test.go` (actor filter at all three role tiers + nil actor fail-closed), `internal/dashboard/errors_test.go` (audit log carries `site_id` + `actor_user_id` on every event).
...
**The IPv4-only LITE BIN (`IP2LOCATION-LITE-DB11.BIN`) ships next to the v4+v6 combined BIN (`IP2LOCATION-LITE-DB11.IPV6.BIN`) in IP2Location's free downloads; installing the wrong one looks fine in spot-checks but silently drops ~50% of streaming-site traffic into `country_code='--'` because that subset is on IPv6. The library returns "IPv6 address missing in IPv4 BIN." for every field on those events, which previously leaked into `events_raw.isp` and `.carrier` alongside the country miss.**

1. **What we did** — On 2026-05-03 (Lesson 30 context) the operator downloaded an IP2Location LITE DB11 BIN to fix 100% `country_code='--'`. The download page shows two DB11 variants next to each other: a smaller IPv4-only `.BIN.ZIP` (~95 MB) and a larger v4+v6 combined `.IPV6.BIN.ZIP` (~213 MB). The smaller file was picked; spot-checks landed on IPv4 visitors and saw real countries (`GB, England, Portsmouth` worked), so the install was declared done. The v0.0.39 ship of the Geo panel surfaced the gap a month later — `televika.com` (site_id=4) showed 49.4% `country_code='--'` over 38 days; every one of those 34,594 rows had `events_raw.isp='IPv6 address missing in IPv4 BIN.'`.
2. **Why it broke** — IP2Location's filename convention puts the IPv4-only variant FIRST in the alphabetic listing and uses the shorter name. The combined v4+v6 variant has `.IPV6.` in the filename (which intuitively reads as "IPv6-only" to a first-time reader, not "v4+v6 combined"). The `airgap-update-geoip.sh` script accepts any DB-named BIN at the hardcoded destination path, so neither it nor the binary itself can tell whether the operator picked the right variant. The PR #88 sentinel filter (`cleanGeoField` stripping "This parameter is unavailable...") was scoped to the LITE-tier-doesn't-carry-this-field case; the IPv4-only-BIN case emits a different sentinel string and wasn't covered.
3. **The fix we applied** — Two-pronged on 2026-06-02. (a) Operational swap: downloaded `IP2LOCATION-LITE-DB11.IPV6.BIN.ZIP`, renamed to `IP2LOCATION-LITE-DB23.BIN` per the hardcoded path convention, `scp -C` to `/var/tmp/`, ran `airgap-update-geoip.sh`. The script's SIGHUP rebind worked cleanly this time (the previous restart-not-reload caveat in Lesson 30 step 7 only applies on initial install when `geoIPPath` was empty at boot). Post-swap measurement: unknown share dropped from 49.4% to 0% on the first batch of post-swap events. (b) Code defense: extended `cleanGeoField` in `internal/enrich/geoip.go` to anchor-match the new sentinel `"IPv6 address missing in IPv4 BIN."` the same way it matches the LITE-tier sentinel. Three new table-driven cases in `cleangeo_internal_test.go` (the sentinel, the sentinel with trailing whitespace, and prose containing "IPv6" that must pass through). All 11 cases green.
4. **Preventive measure** — Lesson 30's runbook entry is amended: the canonical default DB is `IP2LOCATION-LITE-DB11.IPV6.BIN.ZIP` (213 MB combined v4+v6), never `IP2LOCATION-LITE-DB11.BIN.ZIP` (95 MB v4-only). The `geoip-pipeline-review` skill at `.claude/skills/geoip-pipeline-review/` gains a runbook callout naming both filenames explicitly so a future operator can't get the wrong one from a stale wiki link. The `cleanGeoField` change is the second layer of defense: even if a v4-only BIN sneaks back onto the box, the sentinel won't pollute `events_raw.isp` / `.carrier` anymore. Historical IPv6 events stay unresolved (raw IP is discarded at enrichment time per Privacy Rule 1, so they can't be retro-enriched).

### Lesson 39

**A hardcoded `time.LoadLocation("Asia/Tehran")` in `internal/identity/salt.go` is a high-visibility regulator-readable signal that ties the operator to a specific jurisdiction. Any geographic-sensitive constant in a hot-path source file becomes the first thing a compliance reviewer pulls up — even when the technical rationale (rotation aligned to the major customer's operating hours) is sound.**

1. **What we did** — From v0.0.1 through v0.0.38 the salt rotation timezone was a package-level constant: `irst, _ = time.LoadLocation("Asia/Tehran")`. The rotation moment aligned to midnight IRST (UTC+3:30), matching the operating hours of the flagship customer (SamplePlatform, Iranian). Every event's daily salt was derived from `YYYY-MM-DD` in IRST regardless of the customer's actual jurisdiction. The choice was technically defensible (single rotation moment, lowest cross-day-grace cost at a chosen midnight) but visibly Iran-coupled to anyone reading the first 15 lines of `salt.go`.
2. **Why it broke** — The televika.com Path A legal validation (Stream D, June 2026; full report at `jaan-to/outputs/research/03-grc-televika-path-a-residual-risks.md` Section 13) flagged the hardcoded `"Asia/Tehran"` as one of five "operator might be in Iran" signals in the public source. A German compliance reviewer scanning the open-source repo would land on the string within minutes, and the resulting question ("who has admin access to your visitors' data, and where do they sit?") is awkward to answer without a signed residency attestation. The architecture itself was sound; the optics weren't. The same rotation logic, expressed as a per-site configurable field defaulting to UTC, removes the signal entirely without changing behaviour for the Iranian customer (whose `sites.tz` stays `'Asia/Tehran'` and produces byte-identical salts).
3. **The fix we applied** — v0.0.39 (PR fix/salt-rotation-per-site-tz): (a) `SaltManager` methods take `(siteID, tz)` instead of `(siteID)`; (b) the existing `statnive.sites.tz` column (migration 003) is now read by `LookupSitePolicy` and threaded through `RawEvent.TZ` into the pipeline; (c) migration 021 flips the column default from `'Asia/Tehran'` to `'UTC'` (new sites only); (d) empty/invalid tz falls back to UTC at the SaltManager layer (never to IRST); (e) emergency rollback env-var `STATNIVE_SALT_TZ_LEGACY=1` forces every site to `'Asia/Tehran'` byte-identically to v0.0.38; (f) backward-compat snapshot regression test `TestSaltManager_BackwardCompatIRST` pins the SamplePlatform-on-`tz=Asia/Tehran` output so it can never silently drift; (g) SPA `.statnive-tz-chip` in `AppShell.tsx` now reactive to the active site's tz via `tzShortLabel` Intl-based helper (was a static `"Tehran (UTC+03:30)"` label). CLAUDE.md § Stack + Privacy Rule 2 updated; DPA template + draft mirror updated; operator-side `televika-PATH-A-IMPLEMENTATION.md` bumped to v1.4.
4. **Preventive measure** — A new Semgrep rule `salt-no-hardcoded-tz` in `.claude/skills/blake3-hmac-identity-review/semgrep/` fails CI if any new code under `internal/identity/` reintroduces a hardcoded IANA timezone literal (`"Asia/Tehran"`, `"Europe/Berlin"`, etc.) outside the deliberately-named `legacyTimezone` constant. The lesson generalises: **any geographic-sensitive constant in a source file is a regulator signal**. Future jurisdiction-tagged defaults (currency, language, holiday calendar, retention period) belong in `statnive.sites` columns with regulator-safe defaults, not in package-level Go constants. Cross-references: `jaan-to/outputs/research/03-grc-televika-path-a-residual-risks.md` Section 13 (the audit finding), `televika-PATH-A-IMPLEMENTATION.md` Section 13 (the operator-side mitigation).

### Lesson 40

**A new table with a subject-identity column (cookie_id OR user_id) must be wired into a DSAR erase path in the SAME change — and never write a migration comment claiming erasure wiring that does not exist.**

1. **What we did** — PR-E (#192) added migration `023_oauth.sql` with `oauth_auth_codes` + `oauth_refresh_tokens`, each carrying a `user_id UUID` (the consenting dashboard account — PII under GDPR Art. 17). The migration header comment asserted "DSAR erasure deletes WHERE user_id=? (internal/privacy; PR-E extends the user-scoped eraser)" — but PR-E never added that wiring. The only eraser in the branch (`EraseByCookieID`) discovers tables by a `cookie_id` column, so it can never reach a `user_id`-keyed table. The user-scoped account eraser (`EraseByUserID`) lives on a *different* branch (PR-A/#188) and had no knowledge of the new tables.
2. **Why it broke** — Two independent failure modes compounded. (a) The erase path is split by subject: visitor erasure keys on `cookie_id` (this branch), account/user erasure keys on `user_id` (#188) — a new `user_id`-keyed table falls in the gap between them. (b) The migration comment documented *intended* wiring as *done*, so a casual reader (and the author) believed it was covered. Gate 2's 10-agent production-readiness review caught it from THREE dimensions independently (E privacy, H RS-prod, J AS-red-team) — a single reviewer reading the reassuring comment would likely have missed it.
3. **The fix we applied** — Added `privacy.EraseOAuthGrantsByUserID(ctx, userID)` (build-agnostic, `internal/privacy/erase.go`) that runs `ALTER TABLE ... DELETE WHERE user_id = ?` against both grant tables (oauth_clients excluded — operator-owned, not subject PII), with a nil-userID guard so a malformed call can't mass-delete. Unit test for the guard + an integration test (`test/oauth_dsar_integration_test.go`) proving a victim's grants are erased while a bystander's survive. Corrected the migration-023 comment to state the truth: the helper exists but is NOT yet invoked by an account-deletion path on this branch, and #188's `EraseByUserID` MUST call it before the AS is enabled in any GDPR deployment.
4. **Preventive measure** — The `dsar-completeness-checker` skill must assert that **every** table with a subject-identity column (`cookie_id` *or* `user_id`) is reachable by some erase path — not just `cookie_id` tables. Generalised rule for reviewers + `/simplify`: a migration that adds a column linking a row to a data subject is incomplete until the same change-set wires that table into an erase path AND adds a test; and a migration comment may only describe wiring that exists in the same branch (claim-what-you-did, not what-you-intend). Cross-references: Gate 2 run `wf_f9adbd68-bb1` (findings E1/H1/J1/B5/I1); the cross-branch merge requirement is tracked in the `023_oauth.sql` header.
...
**The OpenAPI contract is HYBRID and drift-gated: routes are walked from the chi router, semantics are hand-authored in `api/overlay.yaml`, and `api/openapi.yaml` is GENERATED by merging the two — never hand-edit the merged spec or the skeleton. The router-build had to be extracted to an importable seam (`internal/httpapi.BuildRouter`) first; and the live-verification rung must NEVER POST an event then GET stats (the 1-hour rollup delay makes that a guaranteed false failure).**

1. **What we did** — Added a self-syncing OpenAPI 3.1 contract. The chi router was built inline at the tail of `run()` (`cmd/statnive-live/main.go`), so nothing could walk it; a generator that imported `package main` is impossible. Several constructions in that block (`privacy.NewHandlers`, `spa.Handler`, the `store.Conn()`-backed dep structs) error/dial at construction, so they can't run under the generator's stub deps. Separately, two early instincts for live verification — "POST a synthetic event then GET `/api/stats/overview` and assert the count rose" and "read stats for the self-cleaned probe `site_id=9999`" — would both have failed against a correct server.
2. **Why it broke** — (a) An importable router seam is a prerequisite for deriving routes from code; the inline block couldn't be walked or shared. (b) `chi.Walk` is invisible to middleware ORDER and to non-chi surfaces (OAuth-AS is behind `//go:build chatgpt_app`; MCP-over-HTTP is a separate `http.Server`), so a route-set golden test alone is insufficient. (c) Architecture Rule 3 (1-hour rollup delay) + Rule 1 (raw table is write-only; dashboard reads rollups) mean a freshly-ingested event does NOT appear in `/api/stats/*` for up to an hour; the probe site is self-cleaned to zero rows and never rolled up, so any stats assertion on it is vacuous or a false failure.
3. **The fix we applied** — Extracted `internal/httpapi.BuildRouter(RouterDeps)` (pure wiring; flag-gated error/dial-prone construction stays in `run()` so a flags-OFF boot is byte-identical), guarded by a golden-route-set test + a behavioral middleware-order test + a flags-off boot test. `cmd/specgen` walks the SpecMode router → `api/openapi.gen.yaml`, deep-merges `api/overlay.yaml` → `api/openapi.yaml`. Non-chi surfaces are hand-listed in `coverage_test.go` (`nonChiRoutes`) + the overlay. Schema fidelity is reflected against the Go structs (`TestOverlay_SchemasMatchGoStructs`). Live verification is Prism-proxy of PUBLIC/READ endpoints only (loopback-pinned + fail-closed locally; deploy-gated against prod), and ingestion is confirmed via the `events_raw` ClickHouse oracle, never via stats.
4. **Preventive measure** — `make spec-check` (drift) + `make spec-lint` (redocly + spectral OWASP) + `make spec-breaking` (oasdiff) gate every change; the Go tests ride in `make test`; a repo-local `.claude/settings.json` PreToolUse hook blocks hand-edits to `api/openapi.yaml`/`openapi.gen.yaml`/`web/src/api/generated.ts`. The [`openapi-contract-enforcer`](.claude/skills/openapi-contract-enforcer/README.md) skill encodes the workflow (edit overlay → `make spec-build`). Fuzz/proxy targets refuse any non-loopback target so a per-PR run can never hit production, and exclude every mutating/privacy/admin path. When adding a route: update the overlay, `make spec-build`, and confirm `make test` + `make spec-lint` green — a new undocumented route fails both.
