...
| **7d — Lint + security scans** | ✅ Complete | PR #TBD. Cleared the **185**-finding baseline on main (not ~40 — `wsl_v5` enable + v2.5.0 bump landed after the original estimate): 87 `wsl_v5`, 18 `intrange`, 15 `errcheck`, 13 `revive`, 10 `gofmt`, 9 `paralleltest`, 7 `gosec`, 6 `usestdlibvars`, 4 `perfsprint`, 3 `staticcheck`. `--new-from-rev=origin/main` flag dropped from `ci.yml`; the full-repo lint is now the hard gate. Three new static gates shipped per [`PLAN.md:89`](PLAN.md#L89): (a) `slog-no-raw-pii` Semgrep rule (F3) + fixtures, wired into `make privacy-gate` and the new `semgrep-all` CI job — research backing at [doc 27 §89](../jaan-to/docs/research/27-closing-three-skill-gaps-statnive-live.md); (b) skill-content sanitizer (F6) — bash + perl scanner for Unicode Tag Block (U+E0000–E007F), zero-width (U+200B–200D, U+FEFF), bidi overrides (U+202A–202E, U+2066–2069) across `.claude/skills/**/*.md` + `docs/**/*.md`, research backing at [doc 25 line 69](../jaan-to/docs/research/25-ai-claude-skills-SamplePlatform-grade-analytics-platform.md#L69); (c) `os.Root` migration (F7) — 2 production `os.Open()` sites (`internal/config/secret.go` master secret, `internal/enrich/newvisitor.go` bloom filter LoadFrom + SaveTo) now resolve via `os.OpenRoot` + `root.Open` for TOCTOU-safe loading (openat2 on Linux / OBJECT_ATTRIBUTES on Windows). Symlink-escape regression tests in both packages. New CI workflow `.github/workflows/security-gate.yml` adds 3 jobs: `govulncheck` (Go supply-chain, v1.1.4), `semgrep-all` (all 5 project-local skill rule sets + SARIF upload to Security tab), `codeql` (stock Go + JS + SARIF). All action refs SHA-pinned. SARIF fingerprint baselines (F8) remain deferred to v1.1 per original scope cap. |
