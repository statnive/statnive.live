...
| **v1.1-a11y — WCAG 2.2 AA gate + chart a11y** | ⏳ Pending | `@axe-core/playwright` zero-violation gate + hidden `<table>` mirror on every uPlot chart + keyboard-trap audit against the four-row sticky `AppShell.tsx`. Per [doc 07](../jaan-to/docs/research/07-gaps-wordpress-org-submission-pre‑submission-audit.md) + [doc 35 §5 R13](../jaan-to/docs/research/35-ui-ux-dashboard-evidence-synthesis.md). **Optional prereq to 11a** (internet-facing dashboard = enterprise/legal liability). Recommend folding into 11a's PR. |
...
- **v1.1-a11y — WCAG 2.2 AA gate + chart a11y** ([doc 07](../jaan-to/docs/research/07-gaps-wordpress-org-submission-pre‑submission-audit.md) + [doc 35 §5 R13](../jaan-to/docs/research/35-ui-ux-dashboard-evidence-synthesis.md); ~1 week). Three deliverables: (a) `@axe-core/playwright` integrated into the existing Playwright suite at `web/e2e/` and asserted as zero-violation on every PR touching `web/`; (b) hidden `<table>` mirror or `aria-describedby` summary on every uPlot chart so screen-reader users get the data — uPlot canvas is not natively a11y-readable; (c) keyboard-trap audit pass against the four-row sticky `AppShell.tsx` (sticky-header focus-obscuration is the obvious risk surface). Phase 5e contrast is already AAA (~27:1 navy on paper) — this slice closes the gaps that contrast can't. Bundle gate impact: zero (CI tooling + ARIA-only primitives). **Optional prereq to 11a** — once the dashboard is internet-facing, an undocumented a11y posture is enterprise/legal liability; recommend folding into 11a's PR.
