...
| **5 — Dashboard frontend** | ⛔ Blocked | Preact SPA + uPlot + Frappe Charts. **User-mandated gate: Phase 7b1b + 7b2 (WAL integration + real-traffic verification) must be green first.** |
...
| **7b1b — WAL integration + perf-test tightening** | 🔜 Next | Wire `GroupSyncer` into `wal.go` (`AppendAndWait` API), `handler.go` (handler waits for fsync ack before 202), `pipeline.go` (enrichment becomes inline rather than worker-pool). Consumer changes: ack only after CH commit + retry-on-outage + back-pressure middleware + 30s drain bound. WAL replay structured logging + corrupt-segment audit + same-FS gate + LastIndex monotonic. Tighten crash + ch-outage + disk-full assertions to enforce 0.05% loss SLO. **Hard gate:** `wal-durability-review` skill body (Semgrep rules + 8 fixtures + kill-9 50-runs bash harness). |
| **7b2 — Real-traffic verification + drills** | ⏳ Pending | Real-tracker correctness (queries match expected aggregations from Phase 4 tracker payloads — possible since PR #21 shipped the tracker). Backup restore drill (manual procedure; `clickhouse-backup` script lands in Phase 2c). Manual TLS rotation drill (cert A → SIGHUP → cert B served). Integration-level PII grep (audit log + WAL byte scan for raw user_id) — the unit-level enforcement at `internal/ingest/handler_test.go:TestHandler_UserIDHashedAndCleared` already gates the contract boundary. |
