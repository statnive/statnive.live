...
6. **DNT + GPC default off — operator opt-in per deployment.** Tracker no longer short-circuits client-side on `DNT: 1` / `Sec-GPC: 1` (was hiding 70-85% of Brave / Firefox-strict / Safari traffic from operator dashboards). The browser still attaches the headers automatically; the binary honors them only when `consent.respect_gpc` / `consent.respect_dnt` are flipped to true. Operators with EU visitors **must** opt in; per-site server toggles are the v1.1 surface (currently global config).
