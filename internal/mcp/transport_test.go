...
func TestServeStdio_RoundTrip(t *testing.T) {
	...
	if len(listResp.Result.Tools) != publishedToolCount() {
		t.Errorf("tools/list returned %d tools, want %d (catalog minus reserved)", len(listResp.Result.Tools), publishedToolCount())
	}
}
