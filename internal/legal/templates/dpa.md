...
**OpenAI, Inc. (ChatGPT) is not yet a sub-processor.** The ChatGPT-app integration is gated and unreleased (`mcp.http.profile=chatgpt-app` + `mcp.oauth_as.enabled`, `chatgpt_app` build). When it ships, an end-user who connects the published ChatGPT app authorises OpenAI to read **their own** aggregate analytics over an OAuth token (the read-only MCP surface: no raw IP, no raw `user_id`). Because the customer's end-user initiates and consents to that access on statnive's `/authorize` consent screen, OpenAI will be disclosed as a US-resident (DPF) sub-processor in Schedule A and in the public privacy policy, and re-disclosed here, **before** the app store listing is published. No customer analytics flow to OpenAI until a user explicitly connects ChatGPT.
