...
// merged over this skeleton by `redocly bundle` to produce api/openapi.yaml.
package specgen
...
func Skeleton(routes []Route) []byte {
	...
	b.WriteString("# the merged contract is api/openapi.yaml (redocly bundle).\n")
...
func Merge(skeleton, overlay []byte) ([]byte, error) {
	...
	if skRoot == nil {
		return nil, fmt.Errorf("skeleton has no root mapping")
	}
