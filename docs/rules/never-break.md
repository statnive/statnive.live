...
[14 Security items in CLAUDE.md](../../CLAUDE.md#security-12-features-in-v1-2-phase-1011-deferred). Extended reasoning in [`security-detail.md`](security-detail.md).

| Rule | Source | Enforcement | Blast radius |
|---|---|---|---|
| TLS 1.3 only via manual PEM (`tls.cert_file` / `tls.key_file`); binary never calls ACME | [CLAUDE.md § Security 1](../../CLAUDE.md#security-12-features-in-v1-2-phase-1011-deferred) | `iranian-dc-deploy` skill (`iran-no-letsencrypt-in-binary`) + `air-gap-validator` | Outbound-dependency baked into the air-gap-contracted path |
| ClickHouse bound to 127.0.0.1 only | [CLAUDE.md § Security 2](../../CLAUDE.md#security-12-features-in-v1-2-phase-1011-deferred) | Integration test `security_test.go` | Unauthenticated CH exposed to internet |
| `http.MaxBytesReader` 8KB, field-length caps, ±1h timestamp drift | [CLAUDE.md § Security 4](../../CLAUDE.md#security-12-features-in-v1-2-phase-1011-deferred) | Integration test | Memory-DoS via oversized body; replay attacks |
| Dashboard auth: bcrypt + `crypto/rand` 32-byte session tokens, 14-day TTL, `SameSite=Lax` HttpOnly cookies | [CLAUDE.md § Security 6](../../CLAUDE.md#security-12-features-in-v1-2-phase-1011-deferred) + [PLAN.md Phase 2b](../../PLAN.md) | `internal/auth/nilguard_test.go` (hard gate) | Session fixation; CVE-2024-10924-class bypass |
...
| CGNAT-aware rate-limit tiering — Iranian ASN list (AS44244 Irancell / AS197207 MCI / AS57218 RighTel / AS31549 Shatel / AS43754 Asiatech) on compound `(ip, site_id)` at 1K/s sustained / 2K/s burst; default 100/s elsewhere; per-`site_id` cap 25K/s | [CLAUDE.md § Security 13](../../CLAUDE.md#security-12-features-in-v1-2-phase-1011-deferred) + [doc 27 Gap B](../../../jaan-to/docs/research/27-closing-three-skill-gaps-statnive-live.md) | `ratelimit-tuning-review` skill — **HARD GATE on Phase 10 SamplePlatform cutover** | Iranian mobile CGNAT blanket-throttled → SamplePlatform cutover fails |
| ASN DB is **`iptoasn.com`** public-domain TSV only; MaxMind GeoLite2 + IPLocate rejected (CC-BY-SA) | [CLAUDE.md § Security 13](../../CLAUDE.md#security-12-features-in-v1-2-phase-1011-deferred) | `ratelimit-tuning-review` skill | License rule (§2) violation by proxy |
| Outbound allow-list for opt-in features (ACME, Polar, DB23 download, Telegram, SMTP) routes through `internal/httpclient/guarded.go`: FQDN allow-list + RFC-1918 / CGNAT-100.64 reject after DNS + `https://` forced | [CLAUDE.md § Security 14](../../CLAUDE.md#security-12-features-in-v1-2-phase-1011-deferred) | `airgap-no-raw-httpclient` Semgrep in `air-gap-validator` + unit test | OWASP A10 SSRF; DNS rebinding; air-gap guarantee broken |
| Audit log: `slog` JSONL, file sink only in v1, `O_APPEND`, `Reopen()` for logrotate, `chattr +a` in prod | [CLAUDE.md § Security 10](../../CLAUDE.md#security-12-features-in-v1-2-phase-1011-deferred) | PR review | Audit trail tampering |
| systemd hardening: `NoNewPrivileges`, `ProtectSystem=strict`, `PrivateTmp`, `CapabilityBoundingSet=CAP_NET_BIND_SERVICE`, `SystemCallFilter=@system-service`, `RestrictAddressFamilies` | [CLAUDE.md § Security 12](../../CLAUDE.md#security-12-features-in-v1-2-phase-1011-deferred) | `deploy/systemd/harden-verify.sh` via `make systemd-verify` | Post-RCE escalation; container-escape analog on VPS |
