...
> **MailerLite carve-out.** Added 2026-05-04 alongside the public coming-soon page at [`internal/landing/`](../../internal/landing/). MailerLite's official embed (the verbatim snippet from the MailerLite dashboard — `webforms.min.js` from `groot.mailerlite.com`, fonts from `assets.mlcdn.com`, and a one-time `/takel` form-impression ping to `assets.mailerlite.com`) loads on every visit to `https://statnive.live/`. EU-resident visitors' IPs and User-Agents are visible to MailerLite from the first page-load; submitters additionally transfer their email cross-border (intra-EEA, IE/LT). The Iranian-DC air-gap binary does not register the landing route, so MailerLite is unreachable from any IR-resident code path — no `iran-no-cloudflare`-style sanctions exposure.
